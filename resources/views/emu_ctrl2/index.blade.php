    ...
    <div class="col-lg-12">
        <div class="box">
            <div class="box-header with-border">
                <div class="btn-group">
                    <button onclick="cetakBarcode('{{ route('emu_ctrl2.cetak_barcode') }}')" class="btn btn-success btn-block btn-flat"><i class="fa fa-barcode"></i>Lihat / Cetak Detail</button>
                </div>
            </div>
            <div class="box-body table-responsive">
                <form action="" method="post" class="form-emu_ctrl2">
                    @csrf
...
<script>
    ...
    function editForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Approve & Copy Case Inspeksi');
...
$('#modal-form [name=Tc4_15]').val(response.Tc4_15);
    ...
    function deleteSelected(url) {
        if ($('input:checked').length > 1) {
            if (confirm('Yakin ingin menghapus data terpilih?')) {
                $.post(url, $('.form-emu_ctrl2').serialize())
                    .done((response) => {
                        table.ajax.reload();
                    })
                    .fail((errors) => {
